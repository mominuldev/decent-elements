...
const Optimizer = () => {
	...
	const loadOptimizationSettings = useCallback(async () => {
		try {
			const payload = await api.getOptimization();
			setOptimizationEnabled(payload?.data?.enabled ?? false);
		} catch (error) {
	...
	const loadOptimizationStats = useCallback(async () => {
		try {
			const payload = await api.getOptimizationStats();
	...
	const handleOptimizationToggle = useCallback(
		async (enabled) => {
			try {
				await api.setOptimization(enabled);
				setOptimizationEnabled(enabled);
				showSuccess(
					`Asset optimization ${enabled ? "enabled" : "disabled"} successfully`,
				);
		...
		[showSuccess, showError, loadOptimizationStats],
	);
	...
	const handleGenerateAssets = useCallback(async () => {
		setGenerating(true);
		try {
			await api.generateOptimizedAssets();
	...
	const handleClearAssets = useCallback(async () => {
		if (!window.confirm("Are you sure you want to clear all optimized assets?")) {
			return;
		}

		setClearing(true);
		try {
			await api.clearOptimizedAssets();
	...
	// Load on mount. The callbacks are stable, so this runs once.
	useEffect(() => {
	...
	}, [loadOptimizationSettings, loadOptimizationStats]);
