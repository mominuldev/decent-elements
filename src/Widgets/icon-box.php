...
class Decent_Elements_Icon_Box_Widget extends Widget_Base {
     ...
     *
     * Declaring dependencies lets Elementor enqueue them only on pages where
     * the widget is actually rendered, instead of the plugin loading assets on
     * every Elementor page.
     *
     * @return array
     */
    ...
    protected function register_controls() {
        ...
        $this->add_control(
                'enable_equal_height',
                [
                        'label'        => esc_html__( 'Equal Height?', 'decent-elements' ),
                        'type'         => Controls_Manager::SELECT,
                        'options'      => [
                                'disable' => esc_html__( 'Disable', 'decent-elements' ),
                                'enable'  => esc_html__( 'Enable', 'decent-elements' ),
                        ],
                        'default'      => 'disable',
    ...
    protected function render() {
        ...
        <div <?php $this->print_render_attribute_string( 'box_wrapper' ); ?>>
            ...
            <?php if ( ! empty($settings['box_icon']) || ! empty($settings['icon_shape_image']['url']) ) : ?>
                <div class="de-icon-box__icon-container">
                    ...
                    <?php if ( ! empty( $settings['icon_shape_image']['url'] ) && $settings['icon_bg_shape'] == 'yes' ) : ?>
                        <img class="shape-image" src="<?php echo esc_url( $settings['icon_shape_image']['url'] ); ?>"
                             alt="<?php echo esc_attr( $settings['box_title'] ); ?>">
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            <div class="de-icon-box__content">
                ...
                <?php
                // The title control is a plain TEXT field, so escape it as text
                // before it is composed into the link and tag wrappers below.
                $title = esc_html( $settings['box_title'] );
                ...
                if ( ! empty( $title ) ) {
                    ...
                    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Composed from a validated tag, Elementor-escaped attribute strings and an already-escaped title.
                    echo $title_html;
                }
                ?>
