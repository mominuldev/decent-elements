...
describe('StreamFilter', () => {
    describe('should fail', () => {
        test('if options.filter is not a function', () => {
            try {
                // ts-ignore-next-line
                new StreamFilter(undefined); // eslint-disable-line
    ...
    describe('in object mode', () => {
        describe('should work', () => {
            ...
            test('with no restore option', async () => {
                ...
                const filter = new StreamFilter((obj, _unused, cb) => {
                    if (obj === object2) {
                        return cb(true);
                    }
                    return cb(false);
            ...
            test('with no restore option ans an async filter callback', async () => {
                const inputStream = StreamTest.fromObjects([object1, object2]);
                const filter = new StreamFilter(async (obj) => {
                    if (obj === object2) {
                        return true;
                    }
                    return false;
            ...
            test('with restore option', async () => {
                ...
                const [outputStream, resultPromise] = StreamTest.toObjects();
                const [restoreOutputStream, restoreResultPromise] = StreamTest.toObjects();
                inputStream.pipe(filter).pipe(outputStream);
            ...
            test('with restore and passthrough option in the same pipeline', async () => {
                ...
                duplexStream._write = (obj, _unused, cb) => {
                ...
                inputStream
                    ...
                    .pipe(outputStream);
                outputStream.on('end', () => {
                    assert(passThroughStream1Ended, 'PassThrough stream ends before the output one.');
                    assert(passThroughStream2Ended, 'PassThrough stream ends before the output one.');
                    assert(duplexStreamEnded, 'Duplex stream ends before the output one.');
                });
                filter.restore.on('end', () => {
                    assert(passThroughStream1Ended, 'PassThrough stream ends before the restore one.');
                    assert(passThroughStream2Ended, 'PassThrough stream ends before the restore one.');
                    assert(duplexStreamEnded, 'Duplex stream ends before the restore one.');
                });
    ...
    describe('in buffer mode', () => {
        describe('should work', () => {
            ...
            test('with restore and passthrough option', async () => {
                ...
                restoreInputStream.pipe(filter.restore);
                assert.deepEqual(await resultPromise, buffer1.toString());
                assert.deepEqual(await restoreResultPromise, [buffer3.toString(), buffer2.toString()].join(''));
            });
        });
    });
});
