...
function gulpIconFont(glob, options) {
    options = options || {};
    ...
    const result = multipipe([
        ...
        !!options.autohint &&
            (() => {
                const hintPath = 'string' === typeof options.autohint
                    ? options.autohint
                    : 'ttfautohint';
                const nonTTFfilter = filterStream((file, _, cb) => {
                ...
                return multipipe(nonTTFfilter, spawn({
                    cmd: '/bin/sh',
                    args: [
                        '-c',
