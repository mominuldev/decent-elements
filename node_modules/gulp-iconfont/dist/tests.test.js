...
describe('gulp-iconfont', () => {
    ...
    describe('in stream mode', () => {
        test('should work', async () => {
            ...
            await Promise.all(['iconsfont.ttf', 'iconsfont.woff', 'iconsfont.eot'].map(async (filename, index) => {
                const [contentStream, contentResult] = StreamTest.toChunks();
                files[index].contents.pipe(contentStream);
                expect(Buffer.concat(await contentResult)).toEqual(await readFile(join('fixtures', 'expected', filename)));
            }));
        });
        test('should work for only one font', async () => {
            ...
            await Promise.all(['iconsfont.woff'].map(async (filename, index) => {
                const [contentStream, contentResult] = StreamTest.toChunks();
                files[index].contents.pipe(contentStream);
                expect(Buffer.concat(await contentResult)).toEqual(await readFile(join('fixtures', 'expected', filename)));
            }));
        ...
        test('should output WOFF2 font with woff2 added to formats', async () => {
            ...
            const files = await result;
            expect(files.length).toEqual(3);
            await Promise.all(['iconsfont.ttf', 'iconsfont.woff2', 'iconsfont.woff'].map(async (filename, index) => {
                const [contentStream, contentResult] = StreamTest.toChunks();
                files[index].contents.pipe(contentStream);
                expect(Buffer.concat(await contentResult)).toEqual(await readFile(join('fixtures', 'expected', filename)));
            }));
        ...
        test('should emit an event with the codepoint mapping', async () => {
            ...
            expect(files.length).toEqual(3);
            expect(codepoints).toEqual(JSON.parse(await readFile(join('fixtures', 'expected', 'codepoints.json'), 'utf8')));
        });
        test('should work with autohinted iconsfont', async () => {
            ...
            expect(files.length).toEqual(1);
            const [contentStream, contentResult] = StreamTest.toChunks();
            files[0].contents.pipe(contentStream);
            const contents = Buffer.concat(await contentResult);
            const expected = await readFile(join('fixtures', 'expected', 'hinted', 'iconsfont.ttf'));
    ...
    describe.skip('in buffer mode', () => {
        test('should work', async () => {
            ...
            const files = await result;
            expect(files.length).toEqual(3);
            expect(files[0].contents).toEqual(await readFile(join('fixtures', 'expected', 'iconsfont.ttf')));
            expect(files[1].contents).toEqual(await readFile(join('fixtures', 'expected', 'iconsfont.woff')));
            expect(files[2].contents).toEqual(await readFile(join('fixtures', 'expected', 'iconsfont.eot')));
        });
    });
});
