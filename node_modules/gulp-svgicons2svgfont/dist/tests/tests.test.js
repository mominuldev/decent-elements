...
describe('gulp-svgicons2svgfont', () => {
    ...
    describe('must emit an error', () => {
        test('when a glyph is bad', async () => {
            ...
            try {
                ...
                expect(err.message).toEqual('Non-whitespace before first tag.\nLine: 0\nColumn: 1\nChar: F');
            }
        });
    });
    describe('in stream mode', () => {
        test('should work with cleanicons', async () => {
            ...
            files[0].contents.pipe(contentStream1);
            expect(Buffer.concat(await contentResult1).toString()).toEqual(await readFile(join('fixtures', 'expected', 'test-cleanicons-font.svg'), 'utf-8'));
        });
        ...
        test('should work with prefixedicons', async () => {
            ...
            expect(files[0].isStream()).toEqual(true);
            files[0].contents.pipe(contentStream1);
            expect(Buffer.concat(await contentResult1).toString()).toEqual(await readFile(join('fixtures', 'expected', 'test-prefixedicons-font.svg'), 'utf-8'));
        });
        test('should work with originalicons', async () => {
            ...
            expect(files[0].isStream()).toEqual(true);
            files[0].contents.pipe(contentStream1);
            expect(Buffer.concat(await contentResult1).toString()).toEqual(await readFile(join('fixtures', 'expected', 'test-originalicons-font.svg'), 'utf-8'));
        });
        describe('more', () => {
            beforeEach(async () => {
            ...
            test('should work with unprefixed icons and the prependUnicode option', async () => {
                const [stream, result] = StreamTest.toObjects();
                const [contentStream1, contentResult1] = StreamTest.toChunks();
                svgicons2svgfont(join('fixtures', 'results', 'unprefixedicons', '*.svg'), {
                    fontName: 'unprefixedicons',
                    prependUnicode: true,
                }).pipe(stream);
                const files = await result;
                ...
                expect(files[0].isStream()).toEqual(true);
                files[0].contents.pipe(contentStream1);
                expect(Buffer.concat(await contentResult1).toString()).toEqual(await readFile(join('fixtures', 'expected', 'test-unprefixedicons-font.svg'), 'utf-8'));
                ...
                for (const oldFile of Object.keys(expectedRenames)) {
                    ...
                    const newFilePath = join('fixtures', 'results', 'unprefixedicons', newFile);
                    expect(await readFile(newFilePath)).toBeTruthy();
                    expect(await readFile(join('fixtures', 'results', 'unprefixedicons', newFile), 'utf8')).toEqual(await readFile(join('fixtures', 'icons', 'unprefixedicons', oldFile), 'utf8'));
                }
            });
        });
        describe('more2', () => {
            ...
            test('should work with mixed icons and the prependUnicode option', async () => {
                ...
                expect(files[0].isStream()).toEqual(true);
                files[0].contents.pipe(contentStream1);
                expect(Buffer.concat(await contentResult1).toString()).toEqual(await readFile(join('fixtures', 'expected', 'test-unicons-font.svg'), 'utf-8'));
                const expectedRenames = {
                ...
                for (const oldFile of Object.keys(expectedRenames)) {
                    const newFile = expectedRenames[oldFile];
                    const newFilePath = join('fixtures', 'results', 'unicons', newFile);
                    expect(await readFile(newFilePath)).toBeTruthy();
                    expect(await readFile(join('fixtures', 'results', 'unicons', newFile), 'utf8')).toEqual(await readFile(join('fixtures', 'icons', 'unicons', oldFile), 'utf8'));
                }
            });
        });
        test('should emit an event with the codepoint mapping', async () => {
            ...
            files[0].contents.pipe(contentStream1);
            expect(Buffer.concat(await contentResult1).toString()).toEqual(await readFile(join('fixtures', 'expected', 'test-cleanicons-font.svg'), 'utf-8'));
            expect(codepoints).toEqual(JSON.parse((await readFile(join('fixtures', 'expected', 'test-codepoints.json'))).toString()));
        });
