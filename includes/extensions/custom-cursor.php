...
if (!class_exists('Decent_Elements_Custom_Cursor_Extension')) {
					...
					case 'icon':
						if (!empty($settings['de_custom_cursor_icon']['value'])) {
							...
							if (!empty($settings['de_custom_cursor_icon']['library'])) {
								$element->add_render_attribute('_wrapper', 'data-cursor-icon-library', esc_attr($settings['de_custom_cursor_icon']['library']));
							}
						}
						break;
	...
	new Decent_Elements_Custom_Cursor_Extension();
}